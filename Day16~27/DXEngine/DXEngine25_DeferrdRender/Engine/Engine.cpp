...
bool Engine::InitializeScene()
{
	...
	tppOrenNayar->AddTexture(bearNormalTexture);

	// 배열에 추가.
	meshes.push_back(tppOrenNayar);

	// 메쉬 생성.
	Mesh* tppNormal = new Mesh(fbxTPP, deferredShader);
	...
	// 메쉬 생성.
	Mesh* tppCookTorrance = new Mesh(fbxBear, deferredShader);
	tppCookTorrance->SetPosition(210.0f, -90.0f, 0.0f);
	tppCookTorrance->SetRotation(-90.0f, 180.0f, 0.0f);

	tppCookTorrance->AddTexture(bearDiffuseTexture);
...
{
	deferredBuffer = new DeferrdBuffer();
	if (deferredBuffer->Initialize(device, window->GetScreenWidth(), window->GetScreenHeight()) == false)
		return false;
	...
	{
		Texture newTex;
		newTex.textureResourceView = deferredBuffer->GetShaderResourceView(ix);
	...
	if (deferredRenderer->Initialize(device, deferredMaterial->GetVertexShader()->GetShaderBuffer(), camera, window->GetScreenWidth(), window->GetScreenHeight()) == false)
		return false;
