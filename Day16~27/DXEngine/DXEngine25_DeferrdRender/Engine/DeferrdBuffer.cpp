#include "DeferrdBuffer.h"



DeferrdBuffer::DeferrdBuffer()
{
...
bool DeferrdBuffer::Initialize(ID3D11Device * device, int textureWidth, int textureHeight)
{
...
void DeferrdBuffer::SetRenderTargets(ID3D11DeviceContext * deviceContext, ID3D11DepthStencilView * depthStencilView)
{
	// ·»´õ Å¸°Ù ¼³Á¤. ¹è¿­·Î ¼³Á¤.
	deviceContext->OMSetRenderTargets(BUFFERCOUNT, renderTargetViewArray, depthStencilView);
}

void DeferrdBuffer::ClearRenderTargets(ID3D11DeviceContext* deviceContext, ID3D11DepthStencilView * depthStencilView, float color[])
{
	// ·»´õ Å¸°Ù Áö¿ì±â.
	for (int ix = 0; ix < BUFFERCOUNT; ++ix)
	{
