...
namespace FBXLoader
{
	...
	XMFLOAT3 ReadTangent(FbxMesh * fbxMesh, int controlPointIndex, int vertexCounter)
	{
		...
		// UV 전체 배열 읽어오기.
		FbxGeometryElementTangent* vertexTangent = fbxMesh->GetLayer(0)->GetTangents();
		FbxLayerElement::EReferenceMode referenceMode = vertexTangent->GetReferenceMode();
		FbxLayerElement::EMappingMode mappingMode = vertexTangent->GetMappingMode();
	...
	XMFLOAT3 ReadBinormal(FbxMesh * fbxMesh, int controlPointIndex, int vertexCounter)
	{
		...
		{
			MessageBox(NULL, TEXT("바이노멀이 없습니다"), TEXT("오류"), MB_OK);
			return XMFLOAT3();
		}
		...
		// UV 전체 배열 읽어오기.
		FbxGeometryElementBinormal* vertexBinormal = fbxMesh->GetLayer(0)->GetBinormals();
		FbxLayerElement::EReferenceMode referenceMode = vertexBinormal->GetReferenceMode();
		FbxLayerElement::EMappingMode mappingMode = vertexBinormal->GetMappingMode();
